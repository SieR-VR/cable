...
echo ========================================
echo Cable Audio Driver Build Script
echo ========================================
echo.
...
set MSBUILD_PATHS[3]=C:\Program Files\Microsoft Visual Studio\2022\BuildTools\MSBuild\Current\Bin\MSBuild.exe
set MSBUILD_PATHS[4]=C:\Program Files (x86)\Microsoft Visual Studio\2022\BuildTools\MSBuild\Current\Bin\MSBuild.exe
set MSBUILD_PATHS[5]=C:\Program Files\Microsoft Visual Studio\2019\Community\MSBuild\Current\Bin\MSBuild.exe
set MSBUILD_PATHS[6]=C:\Program Files\Microsoft Visual Studio\2019\Professional\MSBuild\Current\Bin\MSBuild.exe
set MSBUILD_PATHS[7]=C:\Program Files\Microsoft Visual Studio\2019\Enterprise\MSBuild\Current\Bin\MSBuild.exe
set MSBUILD_PATHS[8]=C:\Program Files (x86)\Microsoft Visual Studio\2019\BuildTools\MSBuild\Current\Bin\MSBuild.exe
set MSBUILD_PATHS[9]=C:\Program Files (x86)\MSBuild\14.0\Bin\MSBuild.exe
...
echo Building Cable Audio Driver...
...
if /i "%PLATFORM%"=="ARM64" (
    ...
    call "%MSBUILD_CMD%" "CableAudio.sln" ^
    ...
    call "%MSBUILD_CMD%" "CableAudio.sln" /p:Configuration=%CONFIG% /p:Platform=%PLATFORM% /p:Inf2CatUseLocalTime=true
)
...
for %%c in (%CONFIGS%) do (
    for %%p in (%PLATFORMS%) do (
        ...
        if /i "%%p"=="ARM64" (
            ...
            echo Building x64 with full validation...
            call "%MSBUILD_CMD%" "CableAudio.sln" /p:Configuration=%%c /p:Platform=%%p
        )
...
echo.
echo Key files:
echo   - CableAudio.sys  (driver binary)
echo   - CableAudio.inf  (installation file)
echo   - cableaudio.cat  (catalog file)
