...
#ifndef _CABLEAUDIO_COMMON_H_
#define _CABLEAUDIO_COMMON_H_
...
//
//=============================================================================
typedef struct _CABLEAUDIO_DEVPROPERTY {
    const DEVPROPKEY   *PropertyKey;
    DEVPROPTYPE Type;
    ULONG BufferSize;
    __field_bcount_opt(BufferSize) PVOID Buffer;
} CABLEAUDIO_DEVPROPERTY, PCABLEAUDIO_DEVPROPERTY;
...
typedef struct _ENDPOINT_MINIPAIR 
{
    ...
    // Topology miniport.
    PWSTR                           TopoName;               // make sure this or the template name matches with CABLEAUDIO.<TopoName>.szPname in the inf's [Strings] section
    ...
    const CABLEAUDIO_DEVPROPERTY*       TopoInterfaceProperties;

    // Wave RT miniport.
    PWSTR                           WaveName;               // make sure this or the template name matches with CABLEAUDIO.<WaveName>.szPname in the inf's [Strings] section
    ...
    const CABLEAUDIO_DEVPROPERTY*       WaveInterfaceProperties;
...
DECLARE_INTERFACE_(IAdapterCommon, IUnknown)
{
    ...
    //
    // These methods are called from the IOCTL handler in adapter.cpp
    // to create/remove/rename virtual audio devices at runtime.
    //=====================================================================

    STDMETHOD_(NTSTATUS,        CreateVirtualDevice)
    (
        THIS_
        _In_        PCABLE_DEVICE_CONTROL_PAYLOAD   Payload
    ) PURE;

    STDMETHOD_(NTSTATUS,        RemoveVirtualDevice)
    (
        THIS_
    ...
    STDMETHOD_(NTSTATUS,        MapRingBuffer)
    (
        THIS_
        _In_        const CABLE_DEVICE_ID           DeviceId,
    ...
    STDMETHOD_(NTSTATUS,        UnmapRingBuffer)
    (
        THIS_
        _In_        const CABLE_DEVICE_ID           DeviceId,
    ...
    STDMETHOD_(class CableRingBuffer*,  FindRingBufferByWaveName)
    (
        THIS_
