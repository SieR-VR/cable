...
typedef struct _CABLE_VIRTUAL_DEVICE_ENTRY {
  ...
  UNICODE_STRING
  WaveSymbolicLink; // Symbolic link for KSCATEGORY_AUDIO interface
...
//=============================================================================
static const DEVPROPKEY DEVPKEY_CableAudio_FriendlyName = {
...
//=============================================================================
static const DEVPROPKEY DEVPKEY_CableAudio_DeviceFriendlyName = {
...
//=============================================================================
static const DEVPROPKEY DEVPKEY_CableAudio_Name = {
...
typedef struct _CABLE_INSTALL_WORKITEM_CONTEXT {
  ...
  PENDPOINT_MINIPAIR
  pMiniportPair; // Template (SpeakerMiniports or MicArray1Miniports)
} CABLE_INSTALL_WORKITEM_CONTEXT, *PCABLE_INSTALL_WORKITEM_CONTEXT;

static VOID
CableUnmapRingBufferForEntry(_Inout_ PCABLE_VIRTUAL_DEVICE_ENTRY pEntry) {
  PAGED_CODE();

  if (pEntry == NULL || pEntry->pRingBuffer == NULL ||
  ...
  if (pEntry->pMappingProcess != NULL &&
    ...
    DPF(D_TERSE,
        ("CableUnmapRingBufferForEntry: process mismatch, skipping unmap"));
...
public:
  ...
  STDMETHODIMP_(void) SetWaveServiceGroup(_In_ PSERVICEGROUP ServiceGroup);
  ...
  STDMETHODIMP_(void)
  MixerMuteWrite(_In_ ULONG Index, _In_ ULONG Channel, _In_ BOOL Value);
  ...
  STDMETHODIMP_(void)
  MixerVolumeWrite(_In_ ULONG Index, _In_ ULONG Channel, _In_ LONG Value);

  STDMETHODIMP_(LONG) MixerPeakMeterRead(_In_ ULONG Index, _In_ ULONG Channel);

  STDMETHODIMP_(NTSTATUS)
  WriteEtwEvent(_In_ EPcMiniportEngineEvent miniportEventType,
                _In_ ULONGLONG ullData1, _In_ ULONGLONG ullData2,
                _In_ ULONGLONG ullData3, _In_ ULONGLONG ullData4);

  STDMETHODIMP_(VOID) SetEtwHelper(PPORTCLSETWHELPER _pPortClsEtwHelper);

  STDMETHODIMP_(NTSTATUS)
  InstallSubdevice(
      _In_opt_ PIRP Irp, _In_ PWSTR Name, _In_opt_ PWSTR TemplateName,
      _In_ REFGUID PortClassId, _In_ REFGUID MiniportClassId,
      _In_opt_ PFNCREATEMINIPORT MiniportCreate, _In_ ULONG cPropertyCount,
      _In_reads_opt_(cPropertyCount) const CABLEAUDIO_DEVPROPERTY *pProperties,
      _In_opt_ PVOID DeviceContext, _In_ PENDPOINT_MINIPAIR MiniportPair,
      _In_opt_ PRESOURCELIST ResourceList, _In_ REFGUID PortInterfaceId,
      _Out_opt_ PUNKNOWN *OutPortInterface, _Out_opt_ PUNKNOWN *OutPortUnknown,
      _Out_opt_ PUNKNOWN *OutMiniportUnknown);

  STDMETHODIMP_(NTSTATUS) UnregisterSubdevice(_In_opt_ PUNKNOWN UnknownPort);

  STDMETHODIMP_(NTSTATUS)
  ConnectTopologies(_In_ PUNKNOWN UnknownTopology, _In_ PUNKNOWN UnknownWave,
                    _In_ PHYSICALCONNECTIONTABLE *PhysicalConnections,
                    _In_ ULONG PhysicalConnectionCount);

  STDMETHODIMP_(NTSTATUS)
  DisconnectTopologies(_In_ PUNKNOWN UnknownTopology, _In_ PUNKNOWN UnknownWave,
                       _In_ PHYSICALCONNECTIONTABLE *PhysicalConnections,
                       _In_ ULONG PhysicalConnectionCount);

  STDMETHODIMP_(NTSTATUS)
  InstallEndpointFilters(_In_opt_ PIRP Irp,
  ...
  STDMETHODIMP_(NTSTATUS)
  RemoveEndpointFilters(_In_ PENDPOINT_MINIPAIR MiniportPair,
                        _In_opt_ PUNKNOWN UnknownTopology,
                        _In_opt_ PUNKNOWN UnknownWave);

  STDMETHODIMP_(NTSTATUS)
  GetFilters(_In_ PENDPOINT_MINIPAIR MiniportPair,
  ...
  STDMETHODIMP_(NTSTATUS)
  SetIdlePowerManagement(_In_ PENDPOINT_MINIPAIR MiniportPair,
                         _In_ BOOL bEnabled);
  ...
  //=====================================================================
  STDMETHODIMP_(NTSTATUS)
  CreateVirtualDevice(_In_ PCABLE_DEVICE_CONTROL_PAYLOAD Payload);

  STDMETHODIMP_(NTSTATUS)
  RemoveVirtualDevice(_In_ const CABLE_DEVICE_ID DeviceId);

  //=====================================================================
  // Ring buffer management (Phase 4)
  //=====================================================================
  STDMETHODIMP_(NTSTATUS)
  MapRingBuffer(_In_ const CABLE_DEVICE_ID DeviceId, _Out_ PVOID *UserAddress,
                _Out_ ULONG *TotalSize, _Out_ ULONG *DataBufferSize);

  STDMETHODIMP_(NTSTATUS)
  UnmapRingBuffer(_In_ const CABLE_DEVICE_ID DeviceId, _In_ PVOID UserAddress);

  STDMETHODIMP_(CableRingBuffer *)
  FindRingBufferByWaveName(_In_ PCWSTR WaveName);

  STDMETHODIMP_(CableRingBuffer *)
  FindAnyMappedRingBuffer(_In_ BOOLEAN Capture);

  STDMETHODIMP_(VOID)
  ReleaseRingBufferReference(_In_opt_ CableRingBuffer *RingBuffer);

  //=====================================================================
  // friends
  friend NTSTATUS NewAdapterCommon(_Out_ PUNKNOWN *Unknown, _In_ REFCLSID,
...
private:
  ...
  NTSTATUS GetCachedSubdevice(_In_ PWSTR Name,
                              _Out_opt_ PUNKNOWN *OutUnknownPort,
                              _Out_opt_ PUNKNOWN *OutUnknownMiniport);

  NTSTATUS CacheSubdevice(_In_ PWSTR Name, _In_ PUNKNOWN UnknownPort,
  ...
  NTSTATUS CreateAudioInterfaceWithProperties(
      _In_ PCWSTR ReferenceString, _In_opt_ PCWSTR TemplateReferenceString,
      _In_ ULONG cPropertyCount,
      _In_reads_opt_(cPropertyCount) const CABLEAUDIO_DEVPROPERTY *pProperties,
      _Out_ _At_(AudioSymbolicLinkName->Buffer, __drv_allocatesMem(Mem))
          PUNICODE_STRING AudioSymbolicLinkName);
  ...
  FindVirtualDeviceById(_In_ const CABLE_DEVICE_ID DeviceId);
...
NTSTATUS CableAudioIoSetDeviceInterfacePropertyDataMultiple(
    _In_ PUNICODE_STRING SymbolicLinkName, _In_ ULONG cPropertyCount,
    _In_reads_opt_(cPropertyCount) const CABLEAUDIO_DEVPROPERTY *pProperties) {
  NTSTATUS ntStatus;
  ...
  if (pProperties) {
    for (ULONG i = 0; i < cPropertyCount; i++) {
      ntStatus = IoSetDeviceInterfacePropertyData(
          SymbolicLinkName, pProperties[i].PropertyKey, LOCALE_NEUTRAL,
          PLUGPLAY_PROPERTY_PERSISTENT, pProperties[i].Type,
          pProperties[i].BufferSize, pProperties[i].Buffer);
...
#pragma code_seg("PAGE")
PCABLE_VIRTUAL_DEVICE_ENTRY
CAdapterCommon::FindVirtualDeviceById(_In_ const CABLE_DEVICE_ID DeviceId)
/*++
...
--*/
{
  ...
  for (ULONG i = 0; i < CABLE_MAX_DYNAMIC_DEVICES; i++) {
    if (m_VirtualDevices[i].InUse &&
        RtlCompareMemory(m_VirtualDevices[i].Id, DeviceId,
                         CABLE_DEVICE_ID_SIZE) == CABLE_DEVICE_ID_SIZE) {
...
//=============================================================================
#pragma code_seg("PAGE")
VOID CAdapterCommon::CleanupVirtualDevices()
/*++
...
--*/
{
  ...
  for (ULONG i = 0; i < CABLE_MAX_DYNAMIC_DEVICES; i++) {
    ...
    DPF(D_TERSE, ("CleanupVirtualDevices: removing slot %u '%ws'", i,
                  m_VirtualDevices[i].FriendlyName));
    ...
    if (m_VirtualDevices[i].UnknownTopology != NULL &&
      ...
      DisconnectTopologies(
          m_VirtualDevices[i].UnknownTopology, m_VirtualDevices[i].UnknownWave,
          pTemplate->PhysicalConnections, pTemplate->PhysicalConnectionCount);
    }
    ...
    if (m_VirtualDevices[i].pRingBuffer != NULL) {
      ...
      if (m_VirtualDevices[i].pMappedUserAddress != NULL) {
        DPF(D_TERSE, ("CleanupVirtualDevices: ring buffer mapped in another "
                      "process for slot %u, preserving allocation",
                      i));
        continue;
      }
      ...
      if (m_VirtualDevices[i].pRingBuffer->GetReferenceCount() == 0) {
        ...
        DPF(D_TERSE, ("CleanupVirtualDevices: ring buffer still referenced for "
                      "slot %u, preserving allocation",
                      i));
      }
    }
    ...
    if (m_VirtualDevices[i].WaveSymbolicLink.Buffer != NULL) {
      ...
      RtlZeroMemory(&m_VirtualDevices[i].WaveSymbolicLink,
...
#pragma code_seg("PAGE")
VOID CableInstallEndpointWorkItem(_In_ PDEVICE_OBJECT DeviceObject,
                                  _In_opt_ PVOID Context) {
  UNREFERENCED_PARAMETER(DeviceObject);
  PAGED_CODE();

  PCABLE_INSTALL_WORKITEM_CONTEXT pCtx =
      (PCABLE_INSTALL_WORKITEM_CONTEXT)Context;
  if (pCtx == NULL) {
    return;
  }
  ...
  if (pTemplate->TopoCreateCallback) {
    ntStatus = pTemplate->TopoCreateCallback(
        &topoMiniport, CLSID_PortTopology, NULL, POOL_FLAG_NON_PAGED,
        (PUNKNOWN)pAdapterCommon, NULL, pTemplate);
  } else {
    ntStatus = PcNewMiniport((PMINIPORT *)&topoMiniport, CLSID_PortTopology);
  }
  if (!NT_SUCCESS(ntStatus)) {
    DPF(D_ERROR,
        ("WorkItem: CreateMiniport(Topology) FAILED 0x%08X", ntStatus));
    pCtx->Status = ntStatus;
    goto Done;
  }
  DPF(D_TERSE,
      ("WorkItem: Step 1b - CreateMiniport OK, miniport=%p", topoMiniport));

  DPF(D_TERSE, ("WorkItem: Step 1c - topoPort->Init(Irp=NULL)"));
#pragma warning(push)
#pragma warning(disable : 6387)
  ntStatus =
      topoPort->Init(pAdapterCommon->GetDeviceObject(),
                     NULL, // IRP = NULL for dynamic devices
                     (PMINIPORT)topoMiniport, (PADAPTERCOMMON)pAdapterCommon,
...
#pragma warning(pop)
  ...
  DPF(D_TERSE, ("WorkItem: Step 1d - PcRegisterSubdevice(topo='%ws')",
                pEntry->TopoName));
  ntStatus = PcRegisterSubdevice(pAdapterCommon->GetDeviceObject(),
                                 pEntry->TopoName, topoPort);
  if (!NT_SUCCESS(ntStatus)) {
    DPF(D_ERROR,
        ("WorkItem: PcRegisterSubdevice(topo) FAILED 0x%08X", ntStatus));
    pCtx->Status = ntStatus;
    goto Done;
  }
  ...
  if (pTemplate->WaveCreateCallback) {
    ntStatus = pTemplate->WaveCreateCallback(
        &waveMiniport, CLSID_PortWaveRT, NULL, POOL_FLAG_NON_PAGED,
        (PUNKNOWN)pAdapterCommon, NULL, pTemplate);
  } else {
  ...
  DPF(D_TERSE, ("WorkItem: Step 2b - CreateMiniport(Wave) OK, miniport=%p",
                waveMiniport));
...
#pragma warning(disable : 6387)
  ntStatus =
      wavePort->Init(pAdapterCommon->GetDeviceObject(),
                     NULL, // IRP = NULL for dynamic devices
                     (PMINIPORT)waveMiniport, (PADAPTERCOMMON)pAdapterCommon,
                     NULL // ResourceList = NULL
      );
#pragma warning(pop)
  if (!NT_SUCCESS(ntStatus)) {
  ...
  DPF(D_TERSE, ("WorkItem: Step 2d - PcRegisterSubdevice(wave='%ws')",
                pEntry->WaveName));
  ntStatus = PcRegisterSubdevice(pAdapterCommon->GetDeviceObject(),
                                 pEntry->WaveName, wavePort);
  if (!NT_SUCCESS(ntStatus)) {
    DPF(D_ERROR,
        ("WorkItem: PcRegisterSubdevice(wave) FAILED 0x%08X", ntStatus));
    pCtx->Status = ntStatus;
    goto Done;
  }
  ...
  //
  {
    ...
    DPF(D_TERSE, ("WorkItem: Step 2e - "
                  "IoRegisterDeviceInterface(KSCATEGORY_AUDIO, '%ws')",
                  pEntry->WaveName));

    ntStatus = IoRegisterDeviceInterface(
        pAdapterCommon->GetPhysicalDeviceObject(), &KSCATEGORY_AUDIO,
        &waveRefString, &pEntry->WaveSymbolicLink);

    if (NT_SUCCESS(ntStatus)) {
      DPF(D_TERSE,
          ("WorkItem: Step 2e - IoRegisterDeviceInterface OK, symlink='%wZ'",
           &pEntry->WaveSymbolicLink));

      // Set friendly-name related keys on the interface.
      CABLEAUDIO_DEVPROPERTY friendlyNameProps[3];
      ULONG friendlyNameBytes =
          (ULONG)((wcslen(pEntry->FriendlyName) + 1) * sizeof(WCHAR));
      ...
      NTSTATUS propStatus = CableAudioIoSetDeviceInterfacePropertyDataMultiple(
          &pEntry->WaveSymbolicLink, RTL_NUMBER_OF(friendlyNameProps),
          friendlyNameProps);

      if (!NT_SUCCESS(propStatus)) {
        DPF(D_ERROR,
            ("WorkItem: IoSetDeviceInterfacePropertyData(FriendlyName) FAILED "
             "0x%08X",
             propStatus));
        // Non-fatal: device will work but show generic name
      } else {
        DPF(D_TERSE,
            ("WorkItem: FriendlyName set to '%ws'", pEntry->FriendlyName));
      }
    } else {
      DPF(D_ERROR,
          ("WorkItem: IoRegisterDeviceInterface FAILED 0x%08X", ntStatus));
  ...
  ntStatus = pAdapterCommon->ConnectTopologies(
      topoUnknown, waveUnknown, pTemplate->PhysicalConnections,
  ...
  DPF(D_TERSE, ("WorkItem: SUCCESS topo=%p wave=%p", pEntry->UnknownTopology,
                pEntry->UnknownWave));
...
#pragma code_seg("PAGE")
STDMETHODIMP_(NTSTATUS)
CAdapterCommon::CreateVirtualDevice(_In_ PCABLE_DEVICE_CONTROL_PAYLOAD Payload)
/*++
...
--*/
{
  ...
  if (slotIndex == (ULONG)-1) {
    DPF(D_TERSE, ("CreateVirtualDevice: no free slots (max=%u)",
                  CABLE_MAX_DYNAMIC_DEVICES));
    ExReleaseFastMutex(&m_VirtualDeviceLock);
  ...
  RtlCopyMemory(pEntry->FriendlyName, Payload->FriendlyName,
                sizeof(pEntry->FriendlyName));
  ...
  //
  PENDPOINT_MINIPAIR pTemplate = (Payload->DeviceType == CableDeviceTypeRender)
                                     ? &SpeakerMiniports
                                     : &MicArray1Miniports;
  ...
  DPF(D_TERSE,
      ("CreateVirtualDevice: queuing work item for slot=%u type=%u name='%ws'",
       slotIndex, pEntry->DeviceType, pEntry->FriendlyName));

  IoQueueWorkItem(workItem, CableInstallEndpointWorkItem, DelayedWorkQueue,
                  &ctx);
  ...
  //
  KeWaitForSingleObject(&ctx.CompletionEvent, Executive, KernelMode, FALSE,
                        NULL);
  ...
  if (!NT_SUCCESS(ntStatus)) {
    DPF(D_TERSE,
        ("CreateVirtualDevice: work item failed 0x%08X, rolling back slot %u",
         ntStatus, slotIndex));
    ...
    DPF(D_TERSE,
        ("CreateVirtualDevice: OK slot=%u type=%u wave='%ws' topo=%p wave=%p",
    ...
    if (pEntry->WaveSymbolicLink.Buffer != NULL &&
      ...
      RtlCopyMemory(Payload->WaveSymbolicLink, pEntry->WaveSymbolicLink.Buffer,
                    charCount * sizeof(WCHAR));
      Payload->WaveSymbolicLink[charCount] = L'\0';

      DPF(D_TERSE, ("CreateVirtualDevice: WaveSymbolicLink copied (%u chars)",
                    charCount));
    }
  }
...
#pragma code_seg("PAGE")
STDMETHODIMP_(NTSTATUS)
CAdapterCommon::RemoveVirtualDevice(_In_ const CABLE_DEVICE_ID DeviceId)
/*++
...
--*/
{
  ...
  if (pEntry->pRingBuffer != NULL &&
      pEntry->pRingBuffer->GetReferenceCount() > 0) {
    DPF(D_TERSE, ("RemoveVirtualDevice: ring buffer is in active stream use"));
    ExReleaseFastMutex(&m_VirtualDeviceLock);
    return STATUS_DEVICE_BUSY;
  }

  // If subdevices were registered (future: non-stub path), disconnect and
  // unregister them
  if (pEntry->UnknownTopology != NULL || pEntry->UnknownWave != NULL) {
    // Get the template for physical connections
    PENDPOINT_MINIPAIR pTemplate = (pEntry->DeviceType == CableDeviceTypeRender)
                                       ? &SpeakerMiniports
                                       : &MicArray1Miniports;

    // Disconnect topology <-> wave
    if (pEntry->UnknownTopology != NULL && pEntry->UnknownWave != NULL) {
      DisconnectTopologies(pEntry->UnknownTopology, pEntry->UnknownWave,
...
#pragma code_seg("PAGE")
STDMETHODIMP_(NTSTATUS)
CAdapterCommon::MapRingBuffer(_In_ const CABLE_DEVICE_ID DeviceId,
                              _Out_ PVOID *UserAddress, _Out_ ULONG *TotalSize,
...
--*/
{
  ...
  *DataBufferSize = 0;
  ...
  if (pEntry->pRingBuffer == NULL) {
    pEntry->pRingBuffer =
        new (POOL_FLAG_NON_PAGED, CABLE_RINGBUF_POOLTAG) CableRingBuffer();

    if (pEntry->pRingBuffer == NULL) {
      DPF(D_TERSE,
          ("MapRingBuffer: failed to allocate CableRingBuffer object"));
      ExReleaseFastMutex(&m_VirtualDeviceLock);
      return STATUS_INSUFFICIENT_RESOURCES;
    }

    NTSTATUS ntStatus =
        pEntry->pRingBuffer->Initialize(CABLE_DEFAULT_RING_BUFFER_SIZE);
    if (!NT_SUCCESS(ntStatus)) {
      DPF(D_TERSE,
          ("MapRingBuffer: ring buffer Initialize failed 0x%x", ntStatus));
      delete pEntry->pRingBuffer;
      pEntry->pRingBuffer = NULL;
      ExReleaseFastMutex(&m_VirtualDeviceLock);
      return ntStatus;
  ...
  if (pEntry->pMappedUserAddress != NULL) {
    if (pEntry->pMappingProcess == PsGetCurrentProcess()) {
      ...
      DPF(D_TERSE, ("MapRingBuffer: already mapped at %p in current process",
                    pEntry->pMappedUserAddress));
      ExReleaseFastMutex(&m_VirtualDeviceLock);
      return STATUS_ALREADY_REGISTERED;
    }

    // Different process - stale mapping from a previous app instance that
    // exited without calling UnmapRingBuffer (crash or kill).  Windows already
    // freed that process's user-mode VA on exit, so MmUnmapLockedPages must NOT
    // be called here (user-mode MDL mappings are not tracked in the MDL itself;
    ...
    DPF(D_TERSE, ("MapRingBuffer: clearing stale mapping from previous process "
                  "(addr=%p)",
                  pEntry->pMappedUserAddress));
    if (pEntry->pMappingProcess != NULL) {
      ObDereferenceObject(pEntry->pMappingProcess);
  ...
  *DataBufferSize = pEntry->pRingBuffer->GetDataBufferSize();
  ...
  DPF(D_TERSE, ("MapRingBuffer: OK addr=%p total=%u data=%u", pUserAddr,
                *TotalSize, *DataBufferSize));
...
#pragma code_seg("PAGE")
STDMETHODIMP_(NTSTATUS)
CAdapterCommon::UnmapRingBuffer(_In_ const CABLE_DEVICE_ID DeviceId,
...
#pragma code_seg("PAGE")
STDMETHODIMP_(CableRingBuffer *)
CAdapterCommon::FindRingBufferByWaveName(_In_ PCWSTR WaveName)
/*++
...
--*/
{
  ...
  for (ULONG i = 0; i < CABLE_MAX_DYNAMIC_DEVICES; i++) {
    if (m_VirtualDevices[i].InUse && m_VirtualDevices[i].pRingBuffer != NULL &&
  ...
  if (candidateIndex != (ULONG)-1) {
    ...
    DPF(D_TERSE,
        ("FindRingBufferByWaveName: fallback matched wave '%ws' to slot %u "
         "('%ws')",
...
#pragma code_seg("PAGE")
STDMETHODIMP_(CableRingBuffer *)
CAdapterCommon::FindAnyMappedRingBuffer(_In_ BOOLEAN Capture) {
  PAGED_CODE();
...
#pragma code_seg("PAGE")
NTSTATUS
NewAdapterCommon(_Out_ PUNKNOWN *Unknown, _In_ REFCLSID,
                 _In_opt_ PUNKNOWN UnknownOuter, _In_ POOL_FLAGS PoolFlags)
/*++
...
--*/
{
  ...
  //
  if (InterlockedCompareExchange(&CAdapterCommon::m_AdapterInstances, 1, 0) !=
      0) {
  ...
  //
  CAdapterCommon *p =
      new (PoolFlags, MINADAPTER_POOLTAG) CAdapterCommon(UnknownOuter);
  if (p == NULL) {
...
CAdapterCommon::GetDeviceObject(void)
/*++
...
#pragma code_seg("PAGE")
STDMETHODIMP_(PDEVICE_OBJECT)
CAdapterCommon::GetPhysicalDeviceObject(void)
/*++
...
CAdapterCommon::GetWdfDevice(void)
/*++
...
#pragma code_seg("PAGE")
NTSTATUS
CAdapterCommon::Init(_In_ PDEVICE_OBJECT DeviceObject)
/*++
...
--*/
{
  ...
  IF_FAILED_ACTION_JUMP(
      ntStatus,
      DPF(D_ERROR, ("PcGetPhysicalDeviceObject failed, 0x%x", ntStatus)), Done);
  ...
  //
  ntStatus = WdfDeviceMiniportCreate(WdfGetDriver(), WDF_NO_OBJECT_ATTRIBUTES,
                                     DeviceObject, // FDO
                                     NULL,         // Next device.
                                     NULL,         // PDO
                                     &m_WdfDevice);
  IF_FAILED_ACTION_JUMP(
      ntStatus,
      DPF(D_ERROR, ("WdfDeviceMiniportCreate failed, 0x%x", ntStatus)), Done);
  ...
  CSaveData::SetDeviceObject(
      DeviceObject); // device object is needed by CSaveData
...
Parameter 3: major glitch code: 1:WaveRT buffer is underrun,
                                2:decoder errors,
                                3:receive the same wavert buffer two in a row in
event driven mode Parameter 4: minor code for the glitch cause
...
#pragma code_seg()
STDMETHODIMP
CAdapterCommon::WriteEtwEvent(_In_ EPcMiniportEngineEvent miniportEventType,
                              _In_ ULONGLONG ullData1, _In_ ULONGLONG ullData2,
  ...
  if (m_pPortClsEtwHelper) {
    ntStatus = m_pPortClsEtwHelper->MiniportWriteEtwEvent(
        miniportEventType, ullData1, ullData2, ullData3, ullData4);
  }
  return ntStatus;
} // WriteEtwEvent
...
#pragma code_seg("PAGE")
STDMETHODIMP_(void)
CAdapterCommon::SetEtwHelper(PPORTCLSETWHELPER _pPortClsEtwHelper) {
  PAGED_CODE();
...
#pragma code_seg("PAGE")
STDMETHODIMP
CAdapterCommon::NonDelegatingQueryInterface(_In_ REFIID Interface,
...
#pragma code_seg("PAGE")
STDMETHODIMP_(void)
CAdapterCommon::SetWaveServiceGroup(_In_ PSERVICEGROUP ServiceGroup)
/*++
...
#pragma code_seg()
STDMETHODIMP_(void)
CAdapterCommon::bDevSpecificWrite(_In_ BOOL bDevSpecific)
/*++
...
#pragma code_seg()
STDMETHODIMP_(void)
CAdapterCommon::iDevSpecificWrite(_In_ INT iDevSpecific)
/*++
...
#pragma code_seg()
STDMETHODIMP_(void)
CAdapterCommon::uiDevSpecificWrite(_In_ UINT uiDevSpecific)
/*++
...
#pragma code_seg()
STDMETHODIMP_(BOOL)
CAdapterCommon::MixerMuteRead(_In_ ULONG Index, _In_ ULONG Channel)
/*++
...
#pragma code_seg()
STDMETHODIMP_(void)
CAdapterCommon::MixerMuteWrite(_In_ ULONG Index, _In_ ULONG Channel,
                               _In_ BOOL Value)
/*++
...
#pragma code_seg()
STDMETHODIMP_(void)
CAdapterCommon::MixerMuxWrite(_In_ ULONG Index)
/*++
...
#pragma code_seg()
STDMETHODIMP_(LONG)
CAdapterCommon::MixerVolumeRead(_In_ ULONG Index, _In_ ULONG Channel)
/*++
...
#pragma code_seg()
STDMETHODIMP_(void)
CAdapterCommon::MixerVolumeWrite(_In_ ULONG Index, _In_ ULONG Channel,
                                 _In_ LONG Value)
/*++
...
#pragma code_seg()
STDMETHODIMP_(LONG)
CAdapterCommon::MixerPeakMeterRead(_In_ ULONG Index, _In_ ULONG Channel)
/*++
...
Note:
  ...
  To assist the driver, PortCls will pause any active audio streams prior to
calling this method to place the device in a sleep state. After calling this
method, PortCls will unpause active audio streams, to wake the device up.
Miniports can opt for additional notification by utilizing the IPowerNotify
interface.

  The miniport driver must perform the requested change to the device's power
state before it returns from the PowerChangeState call. If the miniport driver
needs to save or restore any device state before a power-state change, the
miniport driver should support the IPowerNotify interface, which allows it to
receive advance warning of any such change. Before returning from a successful
PowerChangeState call, the miniport driver should cache the new power state.

  While the miniport driver is in one of the sleep states (any state other than
  PowerDeviceD0), it must avoid writing to the hardware. The miniport driver
must cache any hardware accesses that need to be deferred until the device
powers up again. If the power state is changing from one of the sleep states to
PowerDeviceD0, the miniport driver should perform any deferred hardware accesses
after it has powered up the device. If the power state is changing from
PowerDeviceD0 to a sleep state, the miniport driver can perform any necessary
hardware accesses during the PowerChangeState call before it powers down the
device.

  While powered down, a miniport driver is never asked to create a miniport
driver object or stream object. PortCls always places the device in the
PowerDeviceD0 state before calling the miniport driver's NewStream method.
...
--*/
{
  ...
  for (le = m_SubdeviceCache.Flink; le != &m_SubdeviceCache; le = le->Flink) {
    MINIPAIR_UNKNOWN *pRecord =
        CONTAINING_RECORD(le, MINIPAIR_UNKNOWN, ListEntry);
...
#pragma code_seg()
STDMETHODIMP_(NTSTATUS)
CAdapterCommon::QueryDeviceCapabilities(_Inout_updates_bytes_(
    sizeof(DEVICE_CAPABILITIES)) PDEVICE_CAPABILITIES PowerDeviceCaps)
/*++
...
--*/
{
  ...
  // query each miniport for it's power state, we're finished if even one
  // indicates it cannot go to this power state.
  PLIST_ENTRY le = NULL;
  for (le = m_SubdeviceCache.Flink;
       le != &m_SubdeviceCache && NT_SUCCESS(status); le = le->Flink) {
    MINIPAIR_UNKNOWN *pRecord =
        CONTAINING_RECORD(le, MINIPAIR_UNKNOWN, ListEntry);
...
--*/
{
  ...
  //
  ntStatus =
      IoRegisterDeviceInterface(GetPhysicalDeviceObject(), &KSCATEGORY_AUDIO,
                                &referenceString, AudioSymbolicLinkName);

  IF_FAILED_ACTION_JUMP(
      ntStatus,
      DPF(D_ERROR, ("CreateAudioInterfaceWithProperties: "
                    "IoRegisterDeviceInterface(KSCATEGORY_AUDIO): failed, 0x%x",
                    ntStatus)),
      Done);

  //
  // Migrate optional device interface parameters from the template if it exists
  // This is done first, so that any additional parameters in pProperties will
  // override the defaults.
  //
  if (NULL != TemplateReferenceString) {
    ntStatus = MigrateDeviceInterfaceTemplateParameters(
        AudioSymbolicLinkName, TemplateReferenceString);

    IF_FAILED_ACTION_JUMP(
        ntStatus,
        DPF(D_ERROR,
            ("MigrateDeviceInterfaceTempalteParameters: "
             "MigrateDeviceInterfaceTemplateParameters(...): failed, 0x%x",
             ntStatus)),
        Done);
  }
  ...
  //
  ntStatus = CableAudioIoSetDeviceInterfacePropertyDataMultiple(
      AudioSymbolicLinkName, cPropertyCount, pProperties);

  IF_FAILED_ACTION_JUMP(ntStatus,
                        DPF(D_ERROR, ("CreateAudioInterfaceWithProperties: "
                                      "CableAudioIoSetDeviceInterfacePropertyDa"
                                      "taMultiple(...): failed, 0x%x",
                                      ntStatus)),
                        Done);
...
CAdapterCommon::InstallSubdevice(
  ...
  ntStatus = CreateAudioInterfaceWithProperties(
      Name, TemplateName, cPropertyCount, pProperties, &symbolicLink);
  if (NT_SUCCESS(ntStatus)) {
  ...
  if (NT_SUCCESS(ntStatus)) {
    if (MiniportCreate) {
      ntStatus =
          MiniportCreate(&miniport, MiniportClassId, NULL, POOL_FLAG_NON_PAGED,
                         adapterCommon, DeviceContext, MiniportPair);
    } else {
...
#pragma warning(push)
    ...
    // However, for dynamic devices, we may no longer have the resource list and
    // this should still succeed.
    //
#pragma warning(disable : 6387)
    ntStatus =
        port->Init(m_pDeviceObject, Irp, miniport, adapterCommon, ResourceList);
#pragma warning(pop)
...
#pragma code_seg("PAGE")
STDMETHODIMP_(NTSTATUS)
CAdapterCommon::UnregisterSubdevice(_In_opt_ PUNKNOWN UnknownPort)
/*++
...
--*/
{
  ...
  //
  ntStatus = UnknownPort->QueryInterface(IID_IUnregisterSubdevice,
  ...
  if (NT_SUCCESS(ntStatus)) {
    ntStatus =
        unregisterSubdevice->UnregisterSubdevice(m_pDeviceObject, UnknownPort);
...
CAdapterCommon::ConnectTopologies(
    _In_ PUNKNOWN UnknownTopology, _In_ PUNKNOWN UnknownWave,
    _In_ PHYSICALCONNECTIONTABLE *PhysicalConnections,
    _In_ ULONG PhysicalConnectionCount)
/*++
...
--*/
{
  ...
  for (ULONG i = 0; i < PhysicalConnectionCount && NT_SUCCESS(ntStatus); i++) {
    ...
    case CONNECTIONTYPE_TOPOLOGY_OUTPUT:
      ntStatus = PcRegisterPhysicalConnection(
          m_pDeviceObject, UnknownTopology, PhysicalConnections[i].ulTopology,
          UnknownWave, PhysicalConnections[i].ulWave);
      if (!NT_SUCCESS(ntStatus)) {
        DPF(D_TERSE, ("ConnectTopologies: PcRegisterPhysicalConnection(render) "
                      "failed, 0x%x",
                      ntStatus));
      }
      break;
    case CONNECTIONTYPE_WAVE_OUTPUT:
      ntStatus = PcRegisterPhysicalConnection(
          m_pDeviceObject, UnknownWave, PhysicalConnections[i].ulWave,
          UnknownTopology, PhysicalConnections[i].ulTopology);
      if (!NT_SUCCESS(ntStatus)) {
        DPF(D_TERSE, ("ConnectTopologies: "
                      "PcRegisterPhysicalConnection(capture) failed, 0x%x",
                      ntStatus));
      }
      break;
    }
  ...
  if (!NT_SUCCESS(ntStatus)) {
    ...
    DisconnectTopologies(UnknownTopology, UnknownWave, PhysicalConnections,
...
--*/
{
  ...
  //
  ntStatus =
      UnknownTopology->QueryInterface(IID_IUnregisterPhysicalConnection,
  ...
  if (NT_SUCCESS(ntStatus)) {
    for (ULONG i = 0; i < PhysicalConnectionCount; i++) {
      ...
      case CONNECTIONTYPE_TOPOLOGY_OUTPUT:
        ...
        if (!NT_SUCCESS(ntStatus)) {
          DPF(D_TERSE, ("DisconnectTopologies: "
                        "UnregisterPhysicalConnection(render) failed, 0x%x",
                        ntStatus));
        }
        break;
      case CONNECTIONTYPE_WAVE_OUTPUT:
        ...
        if (!NT_SUCCESS(ntStatus2)) {
          DPF(D_TERSE, ("DisconnectTopologies: "
                        "UnregisterPhysicalConnection(capture) failed, 0x%x",
                        ntStatus2));
        }
        break;
      }

      // cache and return the first error encountered, as it's likely the most
      // relevent
      if (NT_SUCCESS(ntStatus)) {
        ntStatus = ntStatus2;
      }
...
#pragma code_seg("PAGE")
NTSTATUS
CAdapterCommon::GetCachedSubdevice(_In_ PWSTR Name,
                                   _Out_opt_ PUNKNOWN *OutUnknownPort,
                                   _Out_opt_ PUNKNOWN *OutUnknownMiniport) {
  PAGED_CODE();
  ...
  for (le = m_SubdeviceCache.Flink; le != &m_SubdeviceCache && !bFound;
       le = le->Flink) {
    MINIPAIR_UNKNOWN *pRecord =
        CONTAINING_RECORD(le, MINIPAIR_UNKNOWN, ListEntry);
...
#pragma code_seg("PAGE")
NTSTATUS
CAdapterCommon::CacheSubdevice(_In_ PWSTR Name, _In_ PUNKNOWN UnknownPort,
                               _In_ PUNKNOWN UnknownMiniport) {
  PAGED_CODE();
  ...
  pNewSubdevice =
      new (POOL_FLAG_NON_PAGED, MINADAPTER_POOLTAG) MINIPAIR_UNKNOWN;
  ...
  if (NT_SUCCESS(ntStatus)) {
    ...
    ntStatus = RtlStringCchCopyW(pNewSubdevice->Name,
                                 SIZEOF_ARRAY(pNewSubdevice->Name), Name);
  }
  ...
  if (NT_SUCCESS(ntStatus)) {
    ...
    // cache the IAdapterPowerManagement interface (if available) from the
    // filter. Some endpoints, like FM and cellular, have their own power
    // requirements that we must track. If this fails, it just means this filter
    // doesn't do power management.
    UnknownMiniport->QueryInterface(IID_IAdapterPowerManagement,
                                    (PVOID *)&(pNewSubdevice->PowerInterface));
...
#pragma code_seg("PAGE")
NTSTATUS
CAdapterCommon::RemoveCachedSubdevice(_In_ PWSTR Name) {
  PAGED_CODE();
  ...
  for (le = m_SubdeviceCache.Flink; le != &m_SubdeviceCache && !bRemoved;
       le = le->Flink) {
    MINIPAIR_UNKNOWN *pRecord =
        CONTAINING_RECORD(le, MINIPAIR_UNKNOWN, ListEntry);
...
#pragma code_seg("PAGE")
VOID CAdapterCommon::EmptySubdeviceCache() {
  PAGED_CODE();
...
CAdapterCommon::InstallEndpointFilters(
    _In_opt_ PIRP Irp, _In_ PENDPOINT_MINIPAIR MiniportPair,
    _In_opt_ PVOID DeviceContext, _Out_opt_ PUNKNOWN *UnknownTopology,
    _Out_opt_ PUNKNOWN *UnknownWave,
    _Out_opt_ PUNKNOWN *UnknownMiniportTopology,
    _Out_opt_ PUNKNOWN *UnknownMiniportWave) {
  PAGED_CODE();
  ...
  ntStatus = GetCachedSubdevice(MiniportPair->TopoName, &unknownTopology,
                                &unknownMiniTopo);
  if (!NT_SUCCESS(ntStatus) || NULL == unknownTopology ||
      NULL == unknownMiniTopo) {
    ...
    ntStatus = InstallSubdevice(
        Irp,
        MiniportPair->TopoName, // make sure this name matches with
                                // CABLEAUDIO.<TopoName>.szPname in the inf's
                                // [Strings] section
        MiniportPair->TemplateTopoName, CLSID_PortTopology, CLSID_PortTopology,
        MiniportPair->TopoCreateCallback,
        MiniportPair->TopoInterfacePropertyCount,
        MiniportPair->TopoInterfaceProperties, DeviceContext, MiniportPair,
        NULL, IID_IPortTopology, NULL, &unknownTopology, &unknownMiniTopo);
    if (NT_SUCCESS(ntStatus)) {
      ntStatus = CacheSubdevice(MiniportPair->TopoName, unknownTopology,
                                unknownMiniTopo);
    }
  }

  ntStatus = GetCachedSubdevice(MiniportPair->WaveName, &unknownWave,
                                &unknownMiniWave);
  if (!NT_SUCCESS(ntStatus) || NULL == unknownWave || NULL == unknownMiniWave) {
    ...
    ntStatus = InstallSubdevice(
        Irp,
        MiniportPair->WaveName, // make sure this name matches with
                                // CABLEAUDIO.<WaveName>.szPname in the inf's
                                // [Strings] section
        MiniportPair->TemplateWaveName, CLSID_PortWaveRT, CLSID_PortWaveRT,
        MiniportPair->WaveCreateCallback,
        MiniportPair->WaveInterfacePropertyCount,
        MiniportPair->WaveInterfaceProperties, DeviceContext, MiniportPair,
        NULL, IID_IPortWaveRT, NULL, &unknownWave, &unknownMiniWave);

    if (NT_SUCCESS(ntStatus)) {
      ntStatus =
          CacheSubdevice(MiniportPair->WaveName, unknownWave, unknownMiniWave);
    }
  }
  ...
  if (unknownTopology && unknownWave) {
    ...
    // miniports.
    //
    ntStatus = ConnectTopologies(unknownTopology, unknownWave,
...
#pragma code_seg("PAGE")
STDMETHODIMP_(NTSTATUS)
CAdapterCommon::RemoveEndpointFilters(_In_ PENDPOINT_MINIPAIR MiniportPair,
                                      _In_opt_ PUNKNOWN UnknownTopology,
                                      _In_opt_ PUNKNOWN UnknownWave) {
  PAGED_CODE();
  ...
  if (UnknownTopology != NULL && UnknownWave != NULL) {
    ntStatus = DisconnectTopologies(UnknownTopology, UnknownWave,
                                    MiniportPair->PhysicalConnections,
                                    MiniportPair->PhysicalConnectionCount);

    if (!NT_SUCCESS(ntStatus)) {
      DPF(D_VERBOSE,
          ("RemoveEndpointFilters: DisconnectTopologies failed: 0x%x",
           ntStatus));
    }
  }
  ...
  if (!NT_SUCCESS(ntStatus)) {
    DPF(D_VERBOSE,
        ("RemoveEndpointFilters: UnregisterSubdevice(wave) failed: 0x%x",
         ntStatus));
  }
  ...
  if (!NT_SUCCESS(ntStatus)) {
    DPF(D_VERBOSE,
        ("RemoveEndpointFilters: UnregisterSubdevice(topology) failed: 0x%x",
         ntStatus));
  }
...
#pragma code_seg("PAGE")
STDMETHODIMP_(NTSTATUS)
CAdapterCommon::GetFilters(_In_ PENDPOINT_MINIPAIR MiniportPair,
                           _Out_opt_ PUNKNOWN *UnknownTopologyPort,
                           _Out_opt_ PUNKNOWN *UnknownTopologyMiniport,
                           _Out_opt_ PUNKNOWN *UnknownWavePort,
                           _Out_opt_ PUNKNOWN *UnknownWaveMiniport) {
  ...
  if (UnknownTopologyPort != NULL || UnknownTopologyMiniport != NULL) {
    ntStatus = GetCachedSubdevice(MiniportPair->TopoName, &unknownTopologyPort,
                                  &unknownTopologyMiniport);
    if (NT_SUCCESS(ntStatus)) {
  ...
  if (NT_SUCCESS(ntStatus) &&
      (UnknownWavePort != NULL || UnknownWaveMiniport != NULL)) {
    ntStatus = GetCachedSubdevice(MiniportPair->WaveName, &unknownWavePort,
                                  &unknownWaveMiniport);
    if (NT_SUCCESS(ntStatus)) {
      if (UnknownWavePort) {
...
#pragma code_seg("PAGE")
STDMETHODIMP_(NTSTATUS)
CAdapterCommon::SetIdlePowerManagement(_In_ PENDPOINT_MINIPAIR MiniportPair,
                                       _In_ BOOL bEnabled) {
  PAGED_CODE();
  ...
  // refcounting disable requests. Each miniport is responsible for calling this
  // in pairs, disable on the first request to disable, enable on the last
  // request to enable.
  ...
  if (NT_SUCCESS(ntStatus)) {
    ntStatus =
        pUnknown->QueryInterface(IID_IPortClsPower, (PVOID *)&pPortClsPower);
  }
...
--*/
{
  ...
  ulFullInfoLength =
      sizeof(KEY_VALUE_FULL_INFORMATION) + MAX_DEVICE_REG_KEY_LENGTH;
  kvFullInfo = (PKEY_VALUE_FULL_INFORMATION)ExAllocatePool2(
      POOL_FLAG_NON_PAGED, ulFullInfoLength, MINADAPTER_POOLTAG);
  IF_TRUE_ACTION_JUMP(kvFullInfo == NULL,
                      ntStatus = STATUS_INSUFFICIENT_RESOURCES, Exit);

  // Iterate over each value and copy it to the destination
  for (UINT i = 0; NT_SUCCESS(ntStatus); i++) {
    // Enumerate the next value
    ntStatus =
        ZwEnumerateValueKey(_hSourceKey, i, KeyValueFullInformation, kvFullInfo,
                            ulFullInfoLength, &ulFullInfoResultLength);

    // Jump out of this loop if there are no more values
    IF_TRUE_ACTION_JUMP(ntStatus == STATUS_NO_MORE_ENTRIES,
                        ntStatus = STATUS_SUCCESS, Exit);

    // Handle incorrect buffer size
    if (ntStatus == STATUS_BUFFER_TOO_SMALL ||
        ntStatus == STATUS_BUFFER_OVERFLOW) {
      // Free and re-allocate the KEY_VALUE_FULL_INFORMATION structure with the
      // correct size
      ...
      kvFullInfo = (PKEY_VALUE_FULL_INFORMATION)ExAllocatePool2(
          POOL_FLAG_NON_PAGED, ulFullInfoLength, MINADAPTER_POOLTAG);
      IF_TRUE_ACTION_JUMP(kvFullInfo == NULL,
                          ntStatus = STATUS_INSUFFICIENT_RESOURCES, loop_exit);

      // Try to enumerate the current value again
      ntStatus = ZwEnumerateValueKey(_hSourceKey, i, KeyValueFullInformation,
                                     kvFullInfo, ulFullInfoLength,
                                     &ulFullInfoResultLength);
    ...
    pwstrKeyValueName = (PWSTR)ExAllocatePool2(
        POOL_FLAG_NON_PAGED, kvFullInfo->NameLength + sizeof(WCHAR) * 2,
        MINADAPTER_POOLTAG);
    IF_TRUE_ACTION_JUMP(pwstrKeyValueName == NULL,
                        ntStatus = STATUS_INSUFFICIENT_RESOURCES, loop_exit);

    // Copy the key value name from the full information struct
    RtlStringCbCopyNW(pwstrKeyValueName,
                      kvFullInfo->NameLength + sizeof(WCHAR) * 2,
                      kvFullInfo->Name, kvFullInfo->NameLength);
    ...
    ntStatus =
        ZwSetValueKey(_hDestinationKey, &strKeyValueName, 0, kvFullInfo->Type,
                      (PVOID)((PUCHAR)kvFullInfo + kvFullInfo->DataOffset),
                      kvFullInfo->DataLength);
    IF_FAILED_JUMP(ntStatus, loop_exit);
...
#pragma code_seg("PAGE")
NTSTATUS
CopyRegistryKey(HANDLE _hSourceKey, HANDLE _hDestinationKey,
                BOOL _bOverwrite = FALSE)
/*++

Routine Description:

  This method recursively copies the registry values in _hSourceKey to
_hDestinationKey. Set _bOverwrite to indicate whether the first level values are
copied or not. Normal use is to set false for the initial call, and then all sub
paths will be copied.
...
--*/
{
  ...
  PAGED_CODE();
  // Validate parameters
  IF_TRUE_ACTION_JUMP(_hSourceKey == nullptr,
                      ntStatus = STATUS_INVALID_PARAMETER, Exit);
  IF_TRUE_ACTION_JUMP(_hDestinationKey == nullptr,
                      ntStatus = STATUS_INVALID_PARAMETER, Exit);

  // Allocate the KEY_BASIC_INFORMATION structure
  ulBasicInfoLength = sizeof(KEY_BASIC_INFORMATION) + MAX_DEVICE_REG_KEY_LENGTH;
  kBasicInfo = (PKEY_BASIC_INFORMATION)ExAllocatePool2(
      POOL_FLAG_NON_PAGED, ulBasicInfoLength, MINADAPTER_POOLTAG);
  IF_TRUE_ACTION_JUMP(kBasicInfo == NULL,
                      ntStatus = STATUS_INSUFFICIENT_RESOURCES, Exit);
  ...
  for (UINT i = 0; NT_SUCCESS(ntStatus); i++) {
    ...
    ntStatus = ZwEnumerateKey(_hSourceKey, i, KeyBasicInformation, kBasicInfo,
                              ulBasicInfoLength, &ulBasicInfoResultLength);

    // Jump out of this loop if there are no more keys
    IF_TRUE_ACTION_JUMP(ntStatus == STATUS_NO_MORE_ENTRIES,
                        ntStatus = STATUS_SUCCESS, copy_values);

    // Handle incorrect buffer size
    if (ntStatus == STATUS_BUFFER_TOO_SMALL ||
        ntStatus == STATUS_BUFFER_OVERFLOW) {
      // Free and re-allocate the KEY_BASIC_INFORMATION structure with the
      // correct size.
    ...
    pwstrKeyName = (PWSTR)ExAllocatePool2(
        POOL_FLAG_NON_PAGED, kBasicInfo->NameLength + sizeof(WCHAR),
        MINADAPTER_POOLTAG);
    IF_TRUE_ACTION_JUMP(pwstrKeyName == NULL,
                        ntStatus = STATUS_INSUFFICIENT_RESOURCES, loop_exit);

    // Copy the key name from the basic information struct
    RtlStringCbCopyNW(pwstrKeyName, kBasicInfo->NameLength + sizeof(WCHAR),
                      kBasicInfo->Name, kBasicInfo->NameLength);
    ...
    InitializeObjectAttributes(&hCurrentSourceKeyAttributes, &strKeyName,
                               OBJ_CASE_INSENSITIVE | OBJ_KERNEL_HANDLE,
                               _hSourceKey, NULL);

    // Open the currently enumerated source key
    ntStatus =
        ZwOpenKey(&hCurrentSourceKey, KEY_READ, &hCurrentSourceKeyAttributes);
    ...
    InitializeObjectAttributes(&hNewDestinationKeyAttributes, &strKeyName,
                               OBJ_KERNEL_HANDLE, _hDestinationKey, NULL);

    // Create the key at the destination
    ntStatus = ZwCreateKey(&hNewDestinationKey, KEY_WRITE,
                           &hNewDestinationKeyAttributes, 0, NULL,
...
Routine Description:
  ...
  For example, if you have an interface in your inf defined with reference
string "SpeakerWave". At runtime you could generate "SpeakerWave-1234ABCDE", and
specify "SpeakerWave" as the template name. When "SpeakerWave-1234ABCDE" is
installed we will copy all of the parameters that were specified in the inf for
"SpeakerWave" over to "SpeakerWave-1234ABCDE". You simply need to specify
"SpeakerWave" as the "TemplateName" in the ENDPOINT_MINIPAIRS.

  By default, the first level of registry keys are not copied. Only the 2nd
level and deeper are copied. This way the friendly name and other PNP properties
will not be modified, but the EP and FX properties will be copied.
...

{
  ...
  //
  // Register an audio interface if not already present for the template
  // interface, so we can access the registry path. If it's already registered,
  // this simply returns the symbolic link name. No need to unregister it (there
  // is no mechanism to), and we'll never make it active.
  //
  ntStatus =
      IoRegisterDeviceInterface(GetPhysicalDeviceObject(), &KSCATEGORY_AUDIO,
                                &referenceString, &TemplateSymbolicLinkName);

  // Open the template device interface's registry key path
  ntStatus =
      IoOpenDeviceInterfaceRegistryKey(&TemplateSymbolicLinkName, GENERIC_READ,
                                       &hTemplateDeviceInterfaceParametersKey);
  IF_FAILED_JUMP(ntStatus, Exit);

  // Open the new device interface's registry key path that we plan to activate
  ntStatus = IoOpenDeviceInterfaceRegistryKey(SymbolicLinkName, GENERIC_WRITE,
                                              &hDeviceInterfaceParametersKey);
  IF_FAILED_JUMP(ntStatus, Exit);

  // Copy the template device parameters key to the device interface key
  ntStatus = CopyRegistryKey(hTemplateDeviceInterfaceParametersKey,
                             hDeviceInterfaceParametersKey);
  IF_FAILED_JUMP(ntStatus, Exit);
