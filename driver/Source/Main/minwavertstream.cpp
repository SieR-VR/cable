...
--*/
{
    ...
    //
    // Phase 4: Try to associate with a Cable ring buffer.
    // Look up by the miniport's cached wave name, which identifies the
    // dynamic virtual device slot. The ring buffer may be NULL if the
    // user hasn't called MAP_RING_BUFFER yet -- that's OK, WriteBytes/
    // ReadBytes handle NULL gracefully (silence / fallback to SaveData).
    //
...
This function writes audio data to the DMA buffer for capture.
If a Cable ring buffer is attached, reads audio data from it.
Otherwise writes silence.
...
--*/
{
    ...
    // read audio data from the ring buffer into the DMA buffer.
    if (m_pCableRingBuffer != NULL && m_pCableRingBuffer->IsInitialized())
    {
        while (ByteDisplacement > 0)
        {
            ULONG runWrite = min(ByteDisplacement, m_ulDmaBufferSize - bufferOffset);
            m_pCableRingBuffer->Read(m_pDmaBuffer + bufferOffset, runWrite);
    ...
    // Fallback: write silence (original behavior for static endpoints)
    while (ByteDisplacement > 0)
    {
        ULONG runWrite = min(ByteDisplacement, m_ulDmaBufferSize - bufferOffset);
...
This function reads audio data from the DMA buffer for render.
If a Cable ring buffer is attached, writes the data into it.
Otherwise saves to file (original behavior).
...
--*/
{
    ...
    // write DMA buffer audio data into the ring buffer for user-mode consumption.
    if (m_pCableRingBuffer != NULL && m_pCableRingBuffer->IsInitialized())
    {
        while (ByteDisplacement > 0)
        {
            ULONG runWrite = min(ByteDisplacement, m_ulDmaBufferSize - bufferOffset);
            m_pCableRingBuffer->Write(m_pDmaBuffer + bufferOffset, runWrite);
            bufferOffset = (bufferOffset + runWrite) % m_ulDmaBufferSize;
            ByteDisplacement -= runWrite;
        }
        return;
    ...
    // Fallback: save to file (original behavior for static endpoints)
    while (ByteDisplacement > 0)
    {
        ULONG runWrite = min(ByteDisplacement, m_ulDmaBufferSize - bufferOffset);
