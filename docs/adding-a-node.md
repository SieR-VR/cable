...
[ ] src/types.ts
    - nodeDefs 객체에 등록 (→ nodeTypes와 serializeNode가 자동으로 갱신됨)
    - NodeType 유니온에 추가
...
> **주의**: `src/nodes/<NodeName>.tsx`의 `toAudioNode`를 빠뜨리거나 `nodeDefs`에 등록하지 않으면, `serializeNode` 호출 시 런타임 오류 또는 Rust 역직렬화 오류가 발생한다.
...
각 노드 파일은 세 가지를 포함해야 한다.

1. **named export** 컴포넌트 함수 (`export function MyNode`)
2. **로컬** `toAudioNode` 함수 (export 없음)
3. **default export** `NodeDefinition` 객체 `{ component, toAudioNode }`
...
// named export: 테스트에서 import { MyNode } from "@/nodes/MyNode" 로 사용
...
// IPC 직렬화 — Rust AudioNode variant의 data 구조와 일치해야 한다
...
// default export: types.ts의 nodeDefs에서 component와 toAudioNode를 함께 가져간다
const definition: NodeDefinition<MyNodeType> = { component: MyNode, toAudioNode };
export default definition;
```

### 4-2. types.ts 등록

`types.ts`는 `nodeDefs` 맵 하나에서 `nodeTypes`와 `serializeNode`를 자동으로 계산한다.
새 노드를 추가할 때 `nodeDefs`에 항목을 추가하는 것만으로 두 곳이 동시에 갱신된다.
...
// nodeTypes와 serializeNode는 nodeDefs에서 자동 계산되므로 별도 수정 불필요
...
### 4-5. state.ts (addNodeAtContextMenu)
...
import { MyNode } from "@/nodes/MyNode";  // named export 사용
...
src/
  node-definition.ts                 NodeDefinition<TNode> 인터페이스 (수정 불필요)
  ipc.d.ts                           커맨드 오버로드 추가 (필요시)
  types.ts                           nodeDefs, NodeType, AudioNode 등록
  state.ts                           addNodeAtContextMenu 타입 + 초기 data
  components/
    ContextMenu.tsx                  NODE_CATEGORIES 항목 추가
  nodes/
    <NodeName>.tsx                   컴포넌트(named) + toAudioNode + NodeDefinition default export (신규)
  test/nodes/
