...
```
crates/tauri/src/driver/endpoint.rs
  snapshot_endpoint_ids()     — 현재 엔드포인트 ID HashSet 반환
  find_new_endpoint_id()      — 폴링으로 새 엔드포인트 탐색 (최대 15회, 300ms 간격)
```
...
- `new_name`: 공백이 포함된 경우 큰따옴표로 감쌈 (`shell_quote()` 참고)
...
crates/tauri/src/driver/endpoint.rs
  set_endpoint_device_desc()          — COM IPropertyStore 쓰기 (elevated 컨텍스트에서 실행)
  CoUninitGuard                       — RAII CoUninitialize 가드
  rename_endpoint_elevated()          — main.rs에서 호출되는 public 진입점
  elevated_set_endpoint_device_desc() — ShellExecuteExW runas + 대기
  shell_quote()                       — 공백 포함 이름 인용부호 처리
```

## 장치 생성 시 전체 흐름

```
create_virtual_device  (driver/commands.rs)
  │
...
```
rename_virtual_device  (driver/commands.rs)
  │
...
| 파일                                | 역할                                           |
| ----------------------------------- | ---------------------------------------------- |
| `crates/tauri/src/driver/endpoint.rs` | 엔드포인트 탐색, COM 쓰기, elevated 헬퍼 구현 |
| `crates/tauri/src/driver/commands.rs` | 가상 장치 생성/삭제/이름 변경 Tauri 커맨드    |
