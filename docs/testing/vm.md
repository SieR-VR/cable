...
└── test/
    ...
    ├── RingBufferDataflow.Tests.ps1     # Pester: 링버퍼 매핑/리드/라이트 데이터 무결성 검증
    ├── E2eMp3FlowHardening.Tests.ps1    # Pester: MP3 재생 → 가상 스피커 → 가상 마이크 종단 루프
...
링 버퍼 매핑 후 사용자 모드에서 직접 읽고 쓴 데이터가 커널 측 PortCls 파이프라인을 거쳐 정확히 보존되는지 검증한다. 데이터 손실/오정렬/포맷 깨짐을 회귀 검사하기 위한 종단 테스트.

### E2eMp3FlowHardening.Tests.ps1

게스트 안에서 실제로 MP3를 재생해 가상 스피커(render)로 흘려보내고, 가상 마이크(capture)에서 원본 신호 특성이 그대로 들어오는지 확인하는 풀-스택 종단 시나리오.
